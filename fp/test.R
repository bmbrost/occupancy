test <- function(Y,W,X,controls,priors,start,n.mcmc){
...
# browser()		
		...
		###
		###  Sample pi (probability of false positive)
		###

		pi <- rbeta(1,pos+priors$a,neg+priors$b)
		
		z1 <- which(z==1)
		for(i in z1){
# i <- z1[1]
			idx1 <- which(Y[i,]==1)
			Y[i,idx1] <- rbinom(length(idx1),1,1-pi)
		}
		Y.long <- c(Y)
...
# browser()
		...
		p.tmp <- psi*apply(p^Y*(1-p)^(1-Y),1,prod)
		p0.tmp <- (1-psi)*apply(pi^Y*(1-pi)^(1-Y),1,prod)
		psi.tmp <- p.tmp/(p.tmp+p0.tmp)
