...
# browser()	
	...
	qW <- ncol(W) 
	M <- tapply(groups$U$subunit,groups$U$unit,function(x) length(unique(x)))  # number of subunits per unit
	qU <- ncol(U)
	R <- length(unique(groups$X$unit))  # number of units
	qX <- ncol(X)
	...
	p <- expit(alpha)  # detection probability
...
# browser()
			...
			p.star <- expit(alpha.star)  # detection probability
			a1 <- a[,i]==1  # update depends only on subunits that are 'used'
		 	mh.star <- sum(dbinom(Y[a1,i],J[a1],p.star,log=TRUE))+
				dnorm(alpha.star,mu.alpha,sigma.alpha,log=TRUE)
		 	mh.0 <- sum(dbinom(Y[a1,i],J[a1],p[i],log=TRUE))+
				dnorm(alpha[1,i],mu.alpha,sigma.alpha,log=TRUE)
			if(exp(mh.star-mh.0) > runif(1)){
				alpha[1,i] <- alpha.star
				p[i] <- p.star
